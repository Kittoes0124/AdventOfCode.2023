...
public static class Day1
{
    ...
    private static Regex DigitRegex => new(
        options: (RegexOptions.Compiled | RegexOptions.IgnoreCase),
        pattern: ("(?=(" + DigitToWordMap.Aggregate(
            func: (pattern, keyValuePair) => (pattern + $"|{keyValuePair.Key}|{keyValuePair.Value}"),
            seed: string.Empty
        )[1..] + "))")
    );
    private static SearchValues<char> DigitSearchValues =>
        SearchValues.Create(values: Enumerable
            .Range(
                count: 10,
                start: 0
            )
            .Select(selector: i => char.ConvertFromUtf32(utf32: (i + 48))[0])
            .ToArray()
        );
    private static ReadOnlyDictionary<string, int> DigitToWordMap =>
    ...
    public static int RunPart1() =>
        Lines.Aggregate(
            func: (sum, line) => {
                var firstDigit = line[index: line.AsSpan().IndexOfAny(values: DigitSearchValues)];
                var lastDigit = line[index: line.AsSpan().LastIndexOfAny(values: DigitSearchValues)];

                return (sum + int.Parse(s: $"{firstDigit}{lastDigit}"));
            },
            seed: 0
        );
    public static int RunPart2() =>
        Lines.Aggregate(
            func: (sum, line) => {
                var firstDigit = 0;
                var lastDigit = 0;
                var matches = DigitRegex.Matches(input: line).Select(selector: match => match.Groups[1].Value);

                foreach (var match in matches) {
                    var digit = ((1 == match.Length) ? int.Parse(s: match) : DigitToWordMap[key: match]);
