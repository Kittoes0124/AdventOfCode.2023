...
public static class Day2
{
    ...
    private static IEnumerable<string> Lines => Input.Split(separator: Environment.NewLine);
    private static IEnumerable<Game> NormalizedGames =>
        Lines
            .Select(selector: line => {
                var gameIdBeginMarkerIndex = line.IndexOf(value: ' ');
                var gameIdEndMarkerIndex = (line.IndexOf(value: ':'));
                var gameId = int.Parse(s: line[(gameIdBeginMarkerIndex + 1)..gameIdEndMarkerIndex]);

                return line[(gameIdEndMarkerIndex + 2)..]
                    .Split(separator: "; ")
                    .SelectMany(selector: handfuls => handfuls.Split(separator: ", "))
                    .Aggregate(
                        func: (game, handful) => {
                            var components = handful.Split(separator: ' ');
                            var count = int.Parse(s: components[0]);
    ...
    public static int RunPart1() =>
        NormalizedGames
            .Where(predicate: counter => (
                (14 >= counter.Blue)
             && (13 >= counter.Green)
             && (12 >= counter.Red)
            ))
            .Aggregate(
                func: (counter, game) => (counter + game.Id),
                seed: 0
            );
    public static int RunPart2() =>
        NormalizedGames.Aggregate(
            func: (counter, game) => (counter + (game.Blue * game.Green * game.Red)),
            seed: 0
        );
}
